...
import { GetHourMetersUseCase, RegisterHourMeterUseCase } from "./hour-meter.usecases";
...
describe("hour-meter use cases", () => {
  ...
  it("registers a reading with the captured operational values", async () => {
    ...
    const result = await new RegisterHourMeterUseCase(repository).execute({
        equipmentId: "Bomba nueva", capturedAt: "2026-08-27T10:00", currentReading: 800, dieselAccumulatedGallons: 15000, dailyMwAccumulated: 12.4, dailyMvarAccumulated: 3.2,
    });
    expect(result.isRight()).toBe(true);
    if (result.isRight()) {
  ...
  it("returns a Left for invalid readings", async () => {
    const result = await new RegisterHourMeterUseCase(new MockHourMeterRepository()).execute({
      equipmentId: "", capturedAt: "", currentReading: 10, dieselAccumulatedGallons: 0, dailyMwAccumulated: 0, dailyMvarAccumulated: 0,
    });
  ...
  it("rejects readings below the selected asset's last values", async () => {
    const result = await new RegisterHourMeterUseCase(new MockHourMeterRepository()).execute({
      equipmentId: "ODO-001", capturedAt: "2026-08-27T10:00", currentReading: 4279,
