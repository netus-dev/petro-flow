...
export interface HourMeterFailure { message: string; fieldErrors?: Partial<Record<"equipmentId" | "capturedAt" | "currentReading" | "dieselAccumulatedGallons" | "dailyMwAccumulated" | "dailyMvarAccumulated", string>>; }
...
export class GetDailyOperationsKpiUseCase {
  ...
  async execute(): Promise<Either<HourMeterFailure, DailyOperationsKpi>> {
    try { return right(await this.repository.getLast24Hours()); }
...
export class RegisterHourMeterUseCase {
  ...
  async execute(input: RegisterHourMeterInput): Promise<Either<HourMeterFailure, HourMeterRecord>> {
    ...
    if (!input.equipmentId.trim()) fieldErrors.equipmentId = "Selecciona un equipo.";
    ...
    try {
      const previous = (await this.repository.getAll()).find((record) => record.id === input.equipmentId);
      if (previous && (input.currentReading < previous.currentReading || input.dieselAccumulatedGallons < previous.dieselAccumulatedGallons || input.dailyMwAccumulated < previous.dailyMwAccumulated || input.dailyMvarAccumulated < previous.dailyMvarAccumulated)) {
        ...
        if (input.dieselAccumulatedGallons < previous.dieselAccumulatedGallons) monotonicErrors.dieselAccumulatedGallons = "No puede ser menor que el último valor.";
        if (input.dailyMwAccumulated < previous.dailyMwAccumulated) monotonicErrors.dailyMwAccumulated = "No puede ser menor que el último valor.";
        if (input.dailyMvarAccumulated < previous.dailyMvarAccumulated) monotonicErrors.dailyMvarAccumulated = "No puede ser menor que el último valor.";
