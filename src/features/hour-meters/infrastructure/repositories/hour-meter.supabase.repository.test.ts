...
describe("Hourmeter infrastructure mapping", () => {
  ...
  it("reads assets using the columns and relations available in the local schema", async () => {
    ...
    await expect(repository.getAll()).resolves.toMatchObject([
       { assetId: "asset-1", equipment: "North Position", platform: "North Position" },
    ]);
    expect(select).toHaveBeenCalledWith("id, current_location_id, functional_principles!assets_function_principle_id_fkey(name), locations!assets_current_location_id_fkey(name), asset_operational_parameters_history!asset_operational_parameters_history_asset_id_fkey(*)");
  });
  ...
  it("registers history with the validated request company", async () => {
    ...
    await repository.register({ assetId: "asset-1", capturedAt: "2026-01-01T00:00:00Z", currentReading: 10, dieselAccumulatedGallons: 100, dailyMwAccumulated: 20, dailyMvarAccumulated: 5 });
    ...
    expect(historySelect).toHaveBeenCalledWith("*, assets!asset_operational_parameters_history_asset_id_fkey(functional_principles!assets_function_principle_id_fkey(name), locations!assets_current_location_id_fkey(name))");
  });
});
